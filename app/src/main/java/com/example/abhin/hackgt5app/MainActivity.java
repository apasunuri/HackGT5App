...
//    private static final String CLOUD_VISION_API_KEY = BuildConfig.API_KEY;
    ...
    HashSet<String> recycablesSet = new HashSet<>();
    HashSet<String> compostingSet = new HashSet<>();
    HashSet<String> technologySet = new HashSet<>();
    String[] recycables = {"paper",
            "newspaper",
            "magazine",
            "catalog",
            "map",
...
//                    return clarifaiClient.recognize(new RecognitionRequest(byteArray).setModel("general-v1.3")).get(0);
                ...
                protected void onPostExecute(ClarifaiResponse<List<ClarifaiOutput<Concept>>> result) {
                    for(int i = 0; i < recycables.length; i++)
                        recycablesSet.add(recycables[i]);
                    for(int i = 0; i < composting.length; i++)
                        compostingSet.add(composting[i]);
                    for(int i = 0; i < technology.length; i++)
                        technologySet.add(technology[i]);
                    boolean found = false;
